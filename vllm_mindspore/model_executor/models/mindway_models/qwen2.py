...
from mindway.transformers.models.qwen2.modeling_qwen2 import Qwen2MLP, Qwen2RMSNorm, Qwen2Attention, Qwen2PreTrainedModel
...
class vLLMQwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        self.layers = nn.CellList(
            [vLLMQwen2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Qwen2ForCausalLM(MindWAYModelBase):
    
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__(vllm_config=vllm_config, prefix=prefix)

        # create model
        qwen2_mindway = vLLMQwen2ForCausalLM.from_pretrained(
            vllm_config.model_config.model,
            mindspore_dtype=mstype.bfloat16
        )
        self.model, self.lm_head = qwen2_mindway.model, qwen2_mindway.lm_head
