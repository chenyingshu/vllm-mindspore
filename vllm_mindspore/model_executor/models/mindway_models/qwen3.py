...
from mindspore import Tensor, mint, nn, ops, jit, mutable
...
from vllm_mindspore.model_executor.models.mindway_models.base import MindWAYModelBase, Fake_Attention
...
from mindway.transformers.models.qwen3.modeling_qwen3 import Qwen3MLP, Qwen3RMSNorm, Qwen3Attention, Qwen3PreTrainedModel
...
class vLLMQwen3Model(Qwen3PreTrainedModel):
    ...
    def __init__(self, config: "Qwen3Config"):
        ...
        self.layers = nn.CellList(
            [vLLMQwen3DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class Qwen3ForCausalLM(MindWAYModelBase):
    
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__(vllm_config=vllm_config, prefix=prefix)

        # create model
        qwen3_mindway = vLLMQwen3ForCausalLM.from_pretrained(
            vllm_config.model_config.model,
            mindspore_dtype=mstype.bfloat16
        )
        self.model, self.lm_head = qwen3_mindway.model, qwen3_mindway.lm_head
    ...
    def forward(
        ...
        **kwargs
        ...
        if is_prefill:
            if not self.prefill:
                self.prefill = True
            enable_dynamic_shape(self.model, *model_inputs)  # enable dynamic shape once on first prefill step
        else:
            if self.prefill:
                self.prefill = False
                enable_dynamic_shape(self.model, *model_inputs)  # enable dynamic shape once on first decode step
    ...
    def load_weights(self, *args, **kwargs) -> Set[str]:        
        if self.config.tie_word_embeddings:
            self.lm_head.weight.set_data(self.model.embed_tokens.embedding_table.data)
    ...
    def compute_logits(
        ...
        if sampling_metadata.selected_token_indices is not None:
            hidden_states = ops.gather(hidden_states, sampling_metadata.selected_token_indices, 0)
