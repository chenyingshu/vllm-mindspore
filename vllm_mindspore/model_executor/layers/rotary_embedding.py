...
class MRotaryEmbedding(RotaryEmbedding):
    ...
    def forward_native(
        ...
        if positions.ndim == 2:
            ...
            # mindspore impl
            # cos_l = cos.split(self.mrope_section, dim=-1)
            # sin_l = sin.split(self.mrope_section, dim=-1)
...
class InferMRotaryEmbedding(InferRotaryEmbedding):
    ...
    def forward_native(
        ...
        if is_prefill:
            # max_pos: 128k, rotary_dim: 128
            # freqs_cos/freqs_cos: (4*max_pos, rotary_dim)
            # Assuming: seq_lens=[1276, 1276], sum(seq_lens) = 2552
            #
            # if positions.ndim == 2:
            #   positions: (3, 2552)
            #   cos/sin: (3, 2552, rotary_dim//2)
            #   cos/sin: cat[(1, 2552, mrope_sec),...] -> (1, 2552, rotary_dim//2)
            # else:
            #   positions: (2552,)
            #   cos/sin: (2552, rotary_dim//2)
        ...
        if positions.ndim == 2 and positions.shape[0] == len(self.mrope_section):
            # max_pos: 128k, rotary_dim: 128
            # freqs_cos/freqs_sin: (4*max_pos, rotary_dim)
            # Assuming: valid_seq_lens=[1277, 1277], sum(seq_lens)=2554, len(self.mrope_section)=3
            #
            # if positions.ndim == 2 and positions.shape[0] == len(self.mrope_section):
            #   positions: (3, 2)
            #   cos/sin: (3, 2, rotary_dim//2)
            #   cos/sin: cat[(2, 1, mrope_sec),...] -> (2, 1, rotary_dim//2)
            #   freqs_cos/freqs_sin: (2, rotary_dim)
            # else:
            #   positions: (1, 1)
            #   cos/sin: (1, 1, rotary_dim//2)
            #   cos/sin: cat[(1, 1, mrope_sec),...] -> (1, 1, rotary_dim//2)
            #   freqs_cos/freqs_sin: (1, rotary_dim//2)
            num_tokens = positions.shape[-1]
            cos, sin = self.freqs_cos[positions], self.freqs_sin[positions]
            cos, sin = cos[..., :self.rotary_dim//2], sin[..., :self.rotary_dim//2]
            cos_l = ops.split(cos, self.mrope_section, axis=-1)
            ...
            # self.freqs_cos/freqs_cos: (4*max_pos, rotary_dim)
            # positions: (2,)
            # freqs_cos/freqs_sin: (2, rotary_dim)
            positions = positions.flatten()
